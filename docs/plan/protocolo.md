...
- **`revision-claude`** falla si la revisión automática del último *commit* tiene hallazgos de corrección: corrígelos, haz *push* y espera la revisión nueva. También falla si la revisión no se completó (cuota agotada, caída del servicio o tiempo agotado): vuelve a ejecutarla con `gh run rerun <id del run> --failed`. Si crees que un hallazgo es un falso positivo, explícalo en el PR y avísale al coordinador. Solo él puede integrar con `gh pr merge <n> --admin --squash`.
